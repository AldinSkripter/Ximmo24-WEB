...
const CookieComponent = () => {
    ...
    return (
        <div className="fixed inset-0 z-[999] flex items-end justify-center bg-slate-950/25 px-0 pt-6 backdrop-blur-[2px] sm:px-5 sm:pb-5" role="dialog" aria-modal="true" aria-labelledby="ximmo-cookie-title">
            <div className="relative w-full max-w-[1180px] overflow-hidden rounded-t-[28px] border border-white/80 bg-white shadow-[0_-18px_70px_rgba(15,23,42,.22)] sm:rounded-[28px]">
                <div className="absolute inset-x-0 top-0 h-1 primaryBg" />
                <div className="relative max-h-[calc(100vh-24px)] overflow-y-auto p-5 sm:max-h-[calc(100vh-40px)] sm:p-7 lg:p-8">
                    ...
                    <div><p className="mt-4 text-sm leading-6 text-slate-600 sm:text-[15px]">{text.description}</p>
                    ...
                    <button type="button" onClick={() => setShowDetails(value => !value)} className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-slate-700 hover:text-slate-950">{text.details}<BiChevronDown className={`transition ${showDetails ? 'rotate-180' : ''}`} /></button></div>
                    <div className="mt-5 lg:mt-0 lg:self-end"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1">
